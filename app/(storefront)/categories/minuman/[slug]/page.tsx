...
export const fetchDrinksByCategory = async (subCategory: string) => {
    const drinks = await db.foods.findMany({
      where: {
        category: "MINUMAN",
...
export default async function drinkCategoryPage({ params }: { params: { slug: string }}) {
  const drinks = await fetchDrinksByCategory(params.slug.replace("-", " "))
  return (
      <section
        id="features"
      ...
      >
        ...
        <div className="grid justify-center gap-4 sm:grid-cols-2 md:max-w-[84rem] md:grid-cols-4">
          {drinks.map((drink) => (
            <div className="flex h-[480px] flex-col justify-between rounded-md p-6 bg-slate-100 dark:bg-slate-800" key={drink.id}>
